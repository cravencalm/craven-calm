...
export default function BookCategoryPage() {
  ...
  const displayName = categoryMap[categorySlug] || (categorySlug ? categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1) : "");
  ...
  return (
    <>
      ...
      <div className="music-page-header" style={{ background: 'linear-gradient(to bottom, rgba(7, 7, 10, 0.98) 0%, rgba(7, 7, 10, 0.8) 100%)' }}>
        <h1 className="music-page-title">{displayName}</h1>
        <p className="music-page-subtitle">Curated volumes from the archives</p>
      </div>
      ...
      <main style={{ paddingBottom: "6rem", minHeight: "60vh" }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '4rem', fontStyle: 'italic', opacity: 0.6 }}>No volumes found in this section of the library.</div>
        ) : (
